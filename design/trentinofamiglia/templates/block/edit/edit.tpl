...
<div id="id_{$block.id}" class="block-container">
    ...
    <div class="Form-field">
        <label class="Form-label u-text-xxs" for="block-name-{$block_id}">{'Name:'|i18n( 'design/standard/block/edit' )}</label>
        <input id="block-name-{$block_id}" class="Form-input u-text-xxs" type="text" name="ContentObjectAttribute_ezpage_block_name_array_{$attribute.id}[{$zone_id}][{$block_id}]" value="{$block.name|wash()}" size="35" />
    </div>

    {if $is_custom|not}
    <div class="Form-field">
        <label class="Form-label u-text-xxs" for="block-overflow-control-{$block_id}">{'Set overflow'|i18n( 'design/standard/block/edit' )}</label>
        <select id="block-overflow-control-{$block_id}" class="Form-input u-text-xxs" name="ContentObjectAttribute_ezpage_block_overflow_{$attribute.id}[{$zone_id}][{$block_id}]">
            <option value=""></option>
    ...
    <div class="block-parameters float-break">
        ...
        {if $is_dynamic}
            ...
            {if eq( $fetch_parameter, 'Source' )}
                ...
                <input id="block-fetch-parameter-choose-source-{$block_id}" class="button u-text-xxs" name="CustomActionButton[{$attribute.id}_new_source_browse-{$zone_id}-{$block_id}]" type="submit" value="{'Choose source'|i18n( 'design/standard/block/edit' )}" />
                {if is_set( $fetch_params['Source'] )}
                    <input id="block-fetch-parameter-remove-source-{$block_id}" class="button u-text-xxs" name="CustomActionButton[{$attribute.id}_remove_source-{$zone_id}-{$block_id}]" type="submit" value="{'Remove source'|i18n( 'design/standard/block/edit' )}" />
                {/if}
            {else}
                <div class="Form-field">
                    <label class="Form-label u-text-xxs">{$fetch_parameter}:</label>
                    <input id="block-fetch-parameter-{$fetch_parameter}-{$block_id}" class="Form-input u-text-xxs" type="text" name="ContentObjectAttribute_ezpage_block_fetch_param_{$attribute.id}[{$zone_id}][{$block_id}][{$fetch_parameter}]" value="{$fetch_params[$fetch_parameter]}" />
                </div>
            {/if}
            {/foreach}
        {/if}
            ...
            {foreach $custom_attributes as $custom_attrib}
                ...
                {if eq( $use_browse_mode[$custom_attrib], 'true' )}
                    <div class="Form-field">
                        ...
                        <input id="block-choose-source-{$block_id}" class="button u-text-xxs" name="CustomActionButton[{$attribute.id}_custom_attribute_browse-{$zone_id}-{$block_id}-{$custom_attrib}]" type="submit" value="{'Choose source'|i18n( 'design/standard/block/edit' )}" />
                        {if is_set( $block.custom_attributes[$custom_attrib] )}
                            <input class="button u-text-xxs" name="CustomActionButton[{$attribute.id}_custom_attribute_remove_source-{$zone_id}-{$block_id}-{$custom_attrib}]" type="submit" value="{'Remove source'|i18n( 'design/standard/block/edit' )}" />
                        {/if}
                    </div>
                {else}
                    <div class="Form-field">
                        ...
                        {if is_set( $custom_attribute_types[$custom_attrib] )}
                            {switch match = $custom_attribute_types[$custom_attrib]}
                                {case match = 'text'}
                                    <textarea id="block-custom_attribute-{$block_id}-{$loop_count}" class="Form-input u-text-xxs" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]" rows="7">{$block.custom_attributes[$custom_attrib]|wash()}</textarea>
                                {/case}
                                {case match = 'checkbox'}
                                    <input id="block-custom_attribute-{$block_id}-{$loop_count}-a" class="Form-input u-text-xxs" type="hidden" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]" value="0" />
                                    <input id="block-custom_attribute-{$block_id}-{$loop_count}-b" class="Form-input u-text-xxs" type="checkbox" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]"{if eq( $block.custom_attributes[$custom_attrib], '1')} checked="checked"{/if} value="1" />
                                {/case}
                                {case match = 'string'}
                                    <input id="block-custom_attribute-{$block_id}-{$loop_count}" class="Form-input u-text-xxs" type="text" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]" value="{$block.custom_attributes[$custom_attrib]|wash()}" />
                                {/case}
                                {case match = 'select'}
                                    {set $custom_attribute_selections = ezini( $block.type, concat( 'CustomAttributeSelection_', $custom_attrib ), 'block.ini' )}
                                    <select id="block-custom_attribute-{$block_id}-{$loop_count}" class="Form-input u-text-xxs" name="ContentObjectAttribute_ezpage_block_custom_attribute_{$attribute.id}[{$zone_id}][{$block_id}][{$custom_attrib}]">
                                ...
                                {case match = 'class_select'}
                                    ...
                                    <select class="select_to_string Form-input u-text-xxs" onchange="select_to_string(this)">
                                        <option></option>
        ...
        {if and( not( $is_dynamic ), not( $is_custom ) )}
            <div class="Form-field">
                <input id="block-add-item-{$block_id}" class="button u-text-xxs" name="CustomActionButton[{$attribute.id}_new_item_browse-{$zone_id}-{$block_id}]" type="submit" value="{'Add item'|i18n( 'design/standard/block/edit' )}" />
            </div>
        {/if}
        </div>
    </div>
    ...
    <div class="block-controls float-break">
        <div class="left">
            <input id="block-remove-selected-{$block_id}" class="button u-text-xxs" type="submit" name="CustomActionButton[{$attribute.id}_remove_item-{$zone_id}-{$block_id}]" value="{'Remove selected'|i18n( 'design/standard/block/edit' )}" />
        </div>
