...
{default html_class='full' placeholder=false()}
  <fieldset
    class="Form-field {if array(2,3)|contains($class_content.selection_type)}Form-field--choose Grid-cell{/if} {if $attribute.has_validation_error} has-error{/if}">
    ...
    {if $contentclass_attribute.description}
      <em
        class="attribute-description">{first_set( $contentclass_attribute.descriptionList[$content_language], $contentclass_attribute.description)|wash}</em>
    {/if}



    {if and($current_user.contentobject.class_identifier|eq('operatore_territoriale'), $current_user.contentobject.data_map.organizzazione.has_content)}


    {def $organization = fetch( 'content', 'object', hash( 'object_id', $current_user.contentobject.data_map.organizzazione.content.relation_list[0].contentobject_id ) )}
    ...
    <fieldset class="Form-field Form-field--choose Grid-cell u-text-r-s">
      ...
      <label class="Form-label Form-label--block">
        <input type="radio" class="Form-input" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]"
               value="{$organization.id}" checked="checked" />
        <span class="Form-fieldIcon"
              role="presentation"></span> {$organization.name}
      </label>
    </fieldset>
    ...
    {if ne( count( $nodesList ), 0)}
      <select class="Form-input" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]">
        {if $attribute.contentclass_attribute.is_required|not}
          <option
            value="no_relation" {if eq( $attribute.content.relation_list|count, 0 )} selected="selected"{/if}></option>
        {/if}
    ...
    {if $attribute.contentclass_attribute.is_required|not}
      <label class="Form-label Form-label--block">
        ...
        <span class="Form-fieldIcon"
              role="presentation"></span> {'No relation'|i18n( 'design/standard/content/datatype' )}
      </label>
    {/if}
    ...
    {section var=node loop=$nodesList}
      <label class="Form-label Form-label--block">
        ...
        <small class="u-text-xxs u-color-grey-40">{$node.url_alias|shorten(70, '...', 'middle')|wash}</small>
      </label>
    {/section}
    {/case}
    ...
    {case match=4}
    {if ne( count( $nodesList ), 0)}
      <select class="{$html_class}" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" size="10"
              multiple>
        {section var=node loop=$nodesList}
          <option value="{$node.contentobject_id}"
            {if ne( count( $attribute.content.relation_list ), 0)}
    ...
    <div class="templatebasedeor">
      ...
      <div class="checkbox"><label>
          <input type="checkbox" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[{$node.node_id}]"
                 value="{$node.contentobject_id}"
            {if ne( count( $attribute.content.relation_list ), 0)}
              {foreach $attribute.content.relation_list as $item}
                {if eq( $item.contentobject_id, $node.contentobject_id )}
      ...
      <div class="templatebasedeor">
        ...
        <div class="radio"><label>
            <input value="no_relation" type="radio"
                   name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" {if eq( $attribute.content.relation_list|count, 0 )} checked="checked"{/if}>{'No relation'|i18n( 'design/standard/content/datatype' )}
            <br/>
          </label></
        <div>
          {/if}
          {section var=node loop=$nodesList}
          <div class="radio"><label>
              <input type="radio" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]"
                     value="{$node.contentobject_id}"
                {if ne( count( $attribute.content.relation_list ), 0)}
                  {foreach $attribute.content.relation_list as $item}
                    {if eq( $item.contentobject_id, $node.contentobject_id )}
