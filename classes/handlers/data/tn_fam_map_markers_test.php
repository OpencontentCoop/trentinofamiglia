...
class DataHandlerTnFamMapMarkersTest implements OpenPADataHandlerInterface
{
  ...
  {
    $this->contentType = eZHTTPTool::instance()->postVariable('contentType', $this->contentType);
  }
  ...
  public function getData()
  {
    ...
    if ($this->contentType == 'geojson') {
      ...
      if (eZHTTPTool::instance()->hasVariable('query') && eZHTTPTool::instance()->hasVariable('attribute')) {
        $this->query = eZHTTPTool::instance()->postVariable('query');
        $this->attributes = explode(',', eZHTTPTool::instance()->postVariable('attribute'));

        return  json_decode( $this->load( md5(trim($this->query . '-' . implode('-', $this->attributes))) ), true ) ;

      }
    } elseif ($this->contentType == 'marker') {
      $view = eZHTTPTool::instance()->postVariable('view', 'panel');
      $id = eZHTTPTool::instance()->postVariable('id', 0);
